...
class Filter extends React.Component {
    ...
    componentDidMount()
    {
        ...
        axios({
          url      :  'http://localhost:4040/filter',
...
handleLocationChange = (event) =>
     {
      const { mealtype, cuisine, page, lcost, hcost, sort } = this.state;
...
{
    const { mealtype,location, cuisine, page, lcost, hcost } = this.state;
...
handleCostChange = (lcost, hcost) =>
{
    ...
    const { mealtype, location, cuisine, page, sort} = this.state;
  ...
  this.props.history.push(`/filter?mealtype=${mealtype}&location=${location}&sort=${sort}&locst=${lcost}&hcost=${hcost}` );
...
return (
            ...
            <div>
                ...
                <div className="heading-search">Breakfast Places in Mumbai</div>
